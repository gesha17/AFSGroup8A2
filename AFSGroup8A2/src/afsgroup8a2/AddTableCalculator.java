 ...
 * @author s165700
 */
public class AddTableCalculator extends Calculator {

    public AddTableCalculator(Operation o) {
        super(o);
    }

    int[][][] doAddTable(int p, int[] modPoly)	{
    	...
    	for (int i = 0; i < n; i++)	{
    		for (int j = 0; j < n; j++)	{
    			result[i][j] = fieldAdd(elements[i], elements[j], p, modPoly);
    		}
    	}
    	return result;
    }
    ...
    @Override
    void calculate() {
        o.answertable = doAddTable(o.p, o.modPoly);
    }
}
